...
export const useERPStore = create<ERPStore>((set, get) => ({
  ...
  deleteItem: async (id) => {
    const res = await fetch(`/api/delete_item/${id}`, { method: 'DELETE' });
    const data = await res.json();
  ...
  addCustomer: async (customer) => {
    const res = await fetch('/api/add_party', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  fetchSales: async () => {
    try {
      ...
      const activeRes = await fetch('/api/active_orders?limit=1000');
  ...
  fetchCustomerOrders: async (email: string, page = 1, limit = 10) => {
    try {
      const res = await fetch(`/api/customer_orders/${encodeURIComponent(email)}?page=${page}&limit=${limit}`);
      const data = await res.json();
      set({
  ...
  addSale: async (sale) => {
    const res = await fetch('/api/save_sale', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
